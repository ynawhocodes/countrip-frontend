...
export const fetchGuideHomeInfo = async () => {
  try {
    const response = await client.get<
      ExternalResponseSuccess<{ existUncheckedReservation: boolean }>
    >("/api/v1/home/info");
    return response;
  } catch (e: any) {
...
export const fetchBookedGuideDatesByCurrentMonth = async (month: string) => {
  try {
    const response = await client.get<
      ExternalResponseSuccess<{ date: string[] }>
...
export const fetchGuideTodaySchedule = async () => {
  try {
    const response = await client.get<
      ExternalResponseSuccess<TodayGuideScheduleDto[]>
...
export const fetchGuideSchedule = async () => {
  try {
    const response = await client.get<
      ExternalResponseSuccess<GuideScheduleDto[]>
    >(`/api/v1/guides/plans`);
    return response;
  } catch (e: any) {
