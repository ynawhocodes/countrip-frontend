...
const SpotItem = ({ data, index }: SpotItemProps) => {
  ...
  return (
    <>
      <StyledContainer>
        <StyledTitle style={fontMedium}>
          {index + 1}. {data.title}
        </StyledTitle>
        {/* TODO: SpotDetailDto 에서 imageUrl, image 혼용 때문에 imageUrl 선택적 필드로 변경으로 인한 sideEffect */}
        {data.imageUrl && <StyledBoxContainer imageUrl={data.imageUrl} />}
        <StyledInfoContainer>
          {InfoTitleList.map((infoTitle, index) => (
            <StyledInfoItemContainer key={index}>
              <StyledInfoTitle style={fontMedium}>{infoTitle}</StyledInfoTitle>
